...
void addOrthogonalMoves(const BoardState& state, Position from,
...
std::vector<Move> rookMoves(const BoardState& state, Position from) {
    std::vector<Move> moves;
    Color color = state.colorAt(from);
    if (color == Color::None) return moves;
    addOrthogonalMoves(state, from, color, moves);
    return moves;
}
} // namespace angry_chess
