...
Move ChessRules::canonicalizeMove(const BoardState& state, const Move& move) const {
    Move m = move;
    if (state.isEmpty(m.from)) return m;
    
    PieceTypeID mover = state.typeAt(m.from);
    Color c = state.colorAt(m.from);
    
    // UX Quality of Life: Castling by clicking King then friendly Rook
    if (mover == KING && state.typeAt(m.to) == ROOK && state.colorAt(m.to) == c) {
        if (m.to.file == 7) m.to.file = 6; // Kingside
        else if (m.to.file == 0) m.to.file = 2; // Queenside
    }
    // UX Quality of Life: En Passant by clicking enemy Pawn beside you
    else if (mover == PAWN && state.typeAt(m.to) == PAWN && state.colorAt(m.to) == opposite(c)) {
        if (m.from.rank == m.to.rank && std::abs(m.from.file - m.to.file) == 1) {
            m.to.rank = (c == Color::White) ? m.from.rank + 1 : m.from.rank - 1;
        }
    }
