...
struct FenParser {
    ...
    static bge::BoardState parse(std::string_view fen);

    /// Generate FEN from a BoardState (requires chess piece semantics).
    static std::string generate(const bge::BoardState& state);
};

} // namespace angry_chess

// Legacy compatibility: keep bge::FenParser as an alias.
namespace bge {
    using FenParser = angry_chess::FenParser;
} // namespace bge
